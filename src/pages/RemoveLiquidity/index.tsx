...
export default function RemoveLiquidity({
  ...
  async function onAttemptToApprove() {
    ...
    const domain = {
      // Pancake to Chicken, i changed to Pancake LPs instead of the current Chicken LPs
      // there is a place that I used chicken as the name around addliquidity, so, i need to locate else, might produce error
      name: 'Chicken LPs',
      version: '1',
      chainId,
                      ...
                      {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsETH ? (
                            ...
                            >
                              {TranslateString(1188, 'Receive WTARA')}
                            </StyledInternalLink>
                          ) : oneCurrencyIsWETH ? (
                            <StyledInternalLink
                              to={`/remove/${
                                currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'TARA' : currencyIdA
                              }/${currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'TARA' : currencyIdB}`}
                            >
                              {TranslateString(1190, 'Receive TARA')}
                            </StyledInternalLink>
                          ) : null}
                        </RowBetween>
                      ) : null}
